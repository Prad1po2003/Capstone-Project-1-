...
def addVehicle():
    ...
    # unique check for LicensePlate ('-' for things like bicycles)
    if plate != "-":
        taken = False
        for v in dataRental:
...
# ---------- RENT ----------
rentalHistory = []  # list of dicts, filled when renting/returning
...
def rentVehicle():
    ...
    # Store simple history (no returns yet)
    rent_record = {
        ...
        "Total": total,
        "Returned": False,         # will flip on return
        "Return Date": ""           # filled on return
    }
